...
-- =====================================================
-- DEVELOPMENT VIEW (uses _dev tables)
-- =====================================================

create or replace view public.ai_receptionist_dashboard_dev_view as
with 
-- Get current date and yesterday for comparisons
date_stats as (
    select 
...
-- Inbound calls statistics (dev table)
inbound_stats as (
    select 
        o.id as organization_id,
        o.name as organization_name,
        ...
        count(*) filter (where ic.created_at::date >= (select fourteen_days_ago from date_stats)) as inbound_calls_last_14_days
    from public.organizations o
    left join public.ai_receptionist_inbound_calls_dev ic on o.id = ic.organization_id
    group by o.id, o.name
),
-- Outbound calls statistics (dev leads table)
outbound_stats as (
    select 
        o.id as organization_id,
        o.name as organization_name,
        ...
        count(*) filter (where l.source = 'vapi_outbound' and l.imported_at::date = (select today from date_stats)) as outbound_calls_completed_today
    from public.organizations o
    left join public.ai_receptionist_leads_dev l on l.source = 'vapi_outbound'  -- Only count VAPI outbound calls
    group by o.id, o.name
),
-- Daily inbound calls trend for last 14 days (dev table)
daily_inbound_trend as (
    select 
        ic.organization_id,
        ic.created_at::date as call_date,
        count(*) as daily_inbound_calls
    from public.ai_receptionist_inbound_calls_dev ic
    where ic.created_at::date >= (select fourteen_days_ago from date_stats)
    group by ic.organization_id, ic.created_at::date
),
-- Daily outbound calls trend for last 14 days (dev table)
daily_outbound_trend as (
    select 
        l.organization_id,
        l.created_at::date as call_date,
        count(*) as daily_outbound_calls
    from public.ai_receptionist_leads_dev l
    where l.source = 'vapi_outbound' and l.created_at::date >= (select fourteen_days_ago from date_stats)
    group by l.organization_id, l.created_at::date
)
-- Main dashboard view
...
-- Development daily trends view
create or replace view public.ai_receptionist_daily_trends_dev_view as
with date_series as (
    select generate_series(
        current_date - interval '13 days',
        current_date,
