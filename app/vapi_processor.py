    ...
    try:
        ...
        for call in outbound_calls:
            try:
                ...
                # Since leads table doesn't have vapi_call_id, we'll check by phone number
                # This is a limitation but works with the current schema
                existing_call = supabase.table(table_name).select("id").eq("phone_number", customer_number).execute()
                ...
                if existing_call.data:
                    ...
                    result = supabase.table(table_name).update(update_fields).eq("phone_number", customer_number).execute()
                    if result.data:
                        updated_count += 1
                        logger.info(f"Updated outbound call for phone: {customer_number}")
                    else:
                        errors.append(f"Failed to update outbound call for phone: {customer_number}")
                else:
                    # Insert new call
                    call_data["created_at"] = datetime.utcnow().isoformat()
                    ...
                    if result.data:
                        inserted_count += 1
                        logger.info(f"Inserted new outbound call for phone: {customer_number}")
                    else:
                        errors.append(f"Failed to insert outbound call for phone: {customer_number}")
